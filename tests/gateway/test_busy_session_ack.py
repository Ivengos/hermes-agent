...
class TestBusySessionAck:
    ...
    async def test_telegram_omits_status_detail_by_default(self):
        ...
        import gateway.run as _gr
        # The display_config fallback for busy_ack_detail is True in
        # _GLOBAL_DEFAULTS, so we have to clear the user config to get
        # the per-platform default (False for telegram).  Without this
        # patch a real user config.yaml with
        # ``display: { busy_ack_detail: true }`` flips Telegram's
        # platform-specific override off and the test starts failing
        # only when run after a test that does set up the user config.
        with patch.object(_gr, "_load_gateway_config", return_value={}):
            runner, sentinel = _make_runner()
            runner._busy_input_mode = "interrupt"
            adapter = _make_adapter()
